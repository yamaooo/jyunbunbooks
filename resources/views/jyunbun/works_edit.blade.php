...
<html lang=ja>
    ...
    <body>
        ...
        <div class="posts_box works_register_box">
            <h2 class="title">作品編集</h2>
            <form action="/works_update/{{$novel->id}}" method="post" enctype="multipart/form-data">
            @csrf
                <div class="posts_wrapper works_register_wrapper">
                    <div class="works_box_first">
                        ...
                        <div class="title_box">
                            ...
                            @enderror
                            <dd>
                                <input type="title" name="title" id="title" maxlength="50" value="{{ $novel->title }}">
                            </dd>
                            ...
                            @enderror
                            <dd>
                                <input type="headline" name="headline" id="headline" maxlength="50" value="{{ $novel->headline }}">
                            </dd>
                            <p>50文字以内でご入力ください</p>
                            ...
                            @foreach ($novels_formats as $novels_format)
                                @if($novels_format->id == $novel->format_id)
                                    <option value="{{ $novels_format->id }}" selected>{{ $novels_format->format }}</option>
                                @else
                    ...
                    <div class="body_box">
                        ...
                        @enderror
                        <dd>
                            <textarea name="body" id="body" maxlength="200000">{{ $novel->body }}</textarea>
                        </dd>
                    ...
                    <dd class="submit_box">
                        <a href="{{ url('/works_user') }}">戻 る</a>
                        <button type="submit" name="submit_btn" id="submit_btn" onclick="return confirm('この内容で更新いたしますか？')">作品更新</button>
                    </dd>
            </form>
        </div>
    </body>
